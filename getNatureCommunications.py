...
def get_project(start,end):
	for i in range(int(start),int(end)):
		...
		if checkRequest.status_code == 200:
			...
			print("---- check NatureCommunications_url status: ",checkRequest.status_code)
			print("---- pdf urls number : ",len(pdfUrlList))
			# print(pdfUrlList)
			NatureCommunicationsUrl = 'https://www.nature.com'
			for pdfurl in pdfUrlList:
				pdfCheckRequest = requests.get(NatureCommunicationsUrl + pdfurl[1])
				print("-------------------------------------------")
				print("---- current page:",str(i))
				print("---- check pdf_url status:",NatureCommunicationsUrl + pdfurl[1],pdfCheckRequest.status_code)
				...
				if len(hitStrFind) != 0 and pdfCheckRequest.status_code == 200:
					for hitstr in hitStrFind[0]:
						...
						if str(hitProject) != 'None':
							...
							if len(S16) != 0:
								urlList = NatureCommunicationsUrl + pdfurl[1] + '\t' + str(hitProject) + '\t16S\n'
							else:
								urlList = NatureCommunicationsUrl + pdfurl[1] + '\t' + str(hitProject) + '\t----\n'
							print("---- writing : ",urlList)
							open('get_papers_project.txt','a',encoding='utf-8').write(urlList)
				else:
					print("---- dont searched fuzzy project in ",NatureCommunicationsUrl + pdfurl[1])
					open('failed_log.txt','a',encoding='utf-8').write("failed until " + NatureCommunicationsUrl + pdfurl[1] + '\n')
				open('run_list.txt', 'a',encoding='utf-8').write(NatureCommunicationsUrl + pdfurl[1] + '\n')
				time.sleep(0.3)
