...
# 2017	s41598-017-01206-4	s41598-017-18356-0	|	1206	18356
# 2018	s41598-017-18550-0	s41598-018-34693-0	|	18550	34693
'''
...
def get_project(year,start,end):
	lastAlphaStr = 'xys1023456789qwertuiopadfghjklzcvbnmQWERTYUIOPASDFGHJKLZXCVBNM'
	for i in range(int(start),int(end)):
		for lastS in [singleAlpha for singleAlpha in lastAlphaStr]:
			checkURL = 'https://www.nature.com/articles/s41598-01' + str(year)[-1] + '-' + str(i).zfill(5) + '-' + lastS
			checkRequest = requests.get(checkURL)
			print("--------------"*6)
			# check request URL
			print("---- check url status: ",checkRequest.status_code)
			print("---- papers : ",str(year) + '-srep-' + str(i).zfill(5) + '-' + lastS)
			hitStrFind = re.findall("(accession n)(.{5,15}[P|E|D|S].{5,90})(\.</)",checkRequest.text)
			if checkRequest.status_code == 200:
				print("---- pdf url existed ")
				if len(hitStrFind) != 0:
